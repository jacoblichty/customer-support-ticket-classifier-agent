...
class TestTicketClassifierAgent:
    ...
    async def test_process_ticket_intelligent_routing(self, agent_with_mock_classifier, sample_ticket):
        ...
        # Check that classification processing metadata is added
        assert hasattr(processed_ticket, 'metadata')
    ...
    async def test_get_statistics_with_data(self, agent_with_mock_classifier, sample_tickets):
        ...
        assert "human_review_needed" in stats
    ...
    def test_classification_insights(self, agent_with_mock_classifier):
        """Test classification insights with and without history."""
        agent = agent_with_mock_classifier
        
        # Test with no history
        insights = agent.get_classification_insights()
        assert insights["total_classifications"] == 0
        assert insights["average_confidence"] == 0.0
        assert insights["category_performance"] == {}
        
        # Add mock processed tickets and test with data
        from ticket_classifier.models import SupportTicket
        mock_ticket = SupportTicket("T001", "Test", "Content", "test@example.com")
        mock_ticket.category = "technical_issue" 
        mock_ticket.confidence_score = 0.9
        agent.processed_tickets = [mock_ticket]
        
        insights = agent.get_classification_insights()
        assert insights["total_classifications"] == 1
        assert insights["average_confidence"] == 0.9
        assert insights["low_confidence_rate"] == 0.0
        assert "technical_issue" in insights["category_performance"]
        assert insights["category_performance"]["technical_issue"]["count"] == 1
